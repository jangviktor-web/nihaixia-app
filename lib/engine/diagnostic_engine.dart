...
class EngineSnapshot {
  ...
  final String? gender;
  final Map<String, int> qAnswers;
  final Set<String> extSymptoms;
  final int flatIndex;
  ...
  EngineSnapshot({
    ...
    this.gender,
    this.qAnswers = const {},
    this.extSymptoms = const {},
    this.flatIndex = 0,
  });
}

class DiagnosticEngine {
  DiagnosticStage _stage = DiagnosticStage.temperaturePattern;
  ...
  // —— 扁平 Q1–Q12 规则引擎接入 ——
  // _qAnswers: {qKey: 选项号(1-based)}；_extSymptoms: 已选扩展症状 key 集合
  Map<String, int> _qAnswers = {};
  Set<String> _extSymptoms = {};
  RuleMatch? _lastRuleMatch;
  ...
  /// 症状匹配（子串语义）：UI 选项文本为「触发词+证候描述（→方剂）」复合串，
  /// 若用 List.contains 精确相等则永不等（根因 A）。改为对列表内任一元素做
  /// 子串包含判定，使「触发词开头」的选项文本可被引擎词命中。
  bool _symSelected(String term) =>
      _selectedSymptoms.any((s) => s.contains(term));
  ...
  EngineSnapshot createSnapshot([int flatIndex = 0]) {
    return EngineSnapshot(
      ...
      gender: _gender,
      qAnswers: Map.from(_qAnswers),
      extSymptoms: Set.from(_extSymptoms),
      flatIndex: flatIndex,
    );
  }
  ...
  void restoreSnapshot(EngineSnapshot snapshot) {
    ...
    _qAnswers = Map.from(snapshot.qAnswers);
    _extSymptoms = Set.from(snapshot.extSymptoms);
  }
  ...
  // ==================== 扁平 Q1–Q12 规则流程（按《问诊公式_全量修正版》） ====================

  /// 扁平问诊序列：Q1 寒热 → Q2 脉(可跳) → Q3 渴(不可跳) → Q4 汗 → Q5 痛
  /// → Q6 便 → Q7 尿 → Q8 胃口 → Q9 睡 → Q10 精神 → Q11 月经(可跳)
  /// → Q12 呕吐(条件触发)。复用 tenQuestions（已按 v3.1 顺序 Q1..Q11 + Q8=⑧追加 Q12）。
  List<FollowUpQuestion> getFlatQuestions({String? defaultGender}) =>
      getTenQuestions(defaultGender: defaultGender);

  /// 按选项索引(0-based)记录 Q 答案（1-based 选项号），同时走原 answerTenQuestion 保留信号副作用。
  void answerFlatQuestion(String key, int optionIndex, String optionText) {
    _qAnswers[key] = optionIndex + 1;
    answerTenQuestion(key, optionText);
  }

  void toggleExtSymptom(String key, bool selected) {
    if (selected) {
      _extSymptoms.add(key);
    } else {
      _extSymptoms.remove(key);
    }
  }

  Set<String> get extSymptoms => Set.unmodifiable(_extSymptoms);
  Map<String, int> get qAnswers => Map.unmodifiable(_qAnswers);
  RuleMatch? get lastRuleMatch => _lastRuleMatch;

  /// 扁平 Q1–Q12 题序编排：基础 Q1..Q11；男性跳过 Q11（月经/性功能）；
  ...
  /// 按规则引擎出方：必选(★)全满足者入候选，按参考(Q)命中打分排序取最佳。
  DiagnosisResult diagnoseByRules() {
    final top = RuleEngine.top(_qAnswers, _extSymptoms);
    _lastRuleMatch = top;
    if (top == null) {
    ...
    final r = top.rule;
    // v3.2 行为调整：必选症状完全相同的多个方剂，同时推荐（不再静默只取一个）。
    final coGroup = RuleEngine.coRecommended(_qAnswers, _extSymptoms);
    final recommended = coGroup.isNotEmpty
        ? coGroup.map((m) => m.rule.name).toList()
        : [r.name];
    final matched = <String>[];
    r.required.forEach((k, v) {
      final ans = _qAnswers[k];
      if (ans != null && v.contains(ans)) {
        matched.add(qOptions[k]?[ans - 1] ?? k);
      }
    });
    ...
    return DiagnosisResult(
      ...
      pattern: '${r.name}证',
      formula: r.name,
      recommendedFormulas: recommended,
      explanation: r.differential,
  ...
  List<FollowUpQuestion> getTenQuestions({String? defaultGender}) {
    ...
    // 如果已设置默认性别，跳过性别问题并自动设置（「性别移出十问」，减少一题）
    if (defaultGender != null && defaultGender.isNotEmpty) {
      _gender = defaultGender;
      questions.removeWhere((q) => q.key == 'gender');
    } else {
      // 未设置默认性别时，重新插入性别题，保留男性跳过月经题的 UX，避免回归
      if (!questions.any((q) => q.key == 'gender')) {
        questions.insert(0, DiagnosticRules.genderQuestion);
      }
    }
    // v3.1：Q8 胃口选「恶心/呕吐」→ 尾部追加 Q12 呕吐类型追问；否则不带
    final appetiteAns = _answers['appetite'];
    if (appetiteAns != null &&
        (appetiteAns.contains('恶心') || appetiteAns.contains('呕吐'))) {
      if (!questions.any((q) => q.key == 'vomit_type')) {
        questions.add(DiagnosticRules.vomitTypeQuestion);
      }
    }
    return questions;
  }
  ...
  void answerTenQuestion(String questionKey, String answer) {
    ...
    // 无损桥接：十问 options 与 formula_rules 的 qOptions 逐项同序，
    // 故用选项在十问题列表中的下标即可对应 canonical 选项号（1-based），
    // 供规则引擎 diagnoseByRules() 主路径读取。fail-closed：找不到则跳过（不误写）。
    _recordFlatAnswer(questionKey, answer);
    ...
    if (questionKey == 'thirst') {
      ...
      // 使少阳信号在定经前就位，不必等跟进问诊，根治柴胡桂枝汤/少阳合病被误判为太阳病的问题。
    ...
    if (questionKey == 'pain') {
      ...
      // v3.1 新增 Q5 选项映射（⑪-⑳）
      _answers['itchy_skin'] = answer.contains('身痒') || answer.contains('皮肤痒');
      _answers['epigastric_hard'] = answer.contains('心下痞硬');
    ...
    if (questionKey == 'vomit_type') {
      ...
      _answers['vomit_profuse'] = answer.contains('剧烈');
      _answers['vomit_immediate'] = answer.contains('食入即吐');
      _answers['vomit_cyclical'] = answer.contains('朝食暮吐');
    }
  ...
  void _detectCombinedPattern() {
    ...
    } else if (primary == '少阳' && sunScore >= 3) {
      // 太阳少阳合病（柴胡桂枝汤）：重算后主经归少阳，但太阳表证（发热恶寒）仍在。
      // 原先 primary=='太阳' 分支因口苦咽干被重判为少阳而失效，此处按合病补回。
      _combinedMeridian = '少阳';
      _combinedPatternCondition = 'sun+shaoyang_combined'; // 柴胡桂枝汤
  ...
  /// 无损桥接：把十问选项文本映射为该 Q 的 canonical 选项号（1-based）写入 _qAnswers。
  /// 依据：kQ1..kQ12 常量 == 十问 key，且 DiagnosticRules.tenQuestions 的 options
  /// 与 formula_rules.qOptions 逐项同序。找不到（indexOf<0）则跳过，绝不误写。
  void _recordFlatAnswer(String questionKey, String answer) {
    if (!qOptions.containsKey(questionKey)) return; // 仅 kQ1..kQ12 进入规则引擎
    List<String>? opts;
    for (final q in DiagnosticRules.tenQuestions) {
      if (q.key == questionKey) {
        opts = q.options;
        break;
      }
    }
    if (questionKey == kQ12) opts = DiagnosticRules.vomitTypeQuestion.options;
    if (opts == null) return;
    final idx = opts.indexOf(answer);
    if (idx < 0) return; // fail-closed
    _qAnswers[questionKey] = idx + 1;
  }

  /// 把十问/跟进已捕获的布尔标志同步为规则引擎的扩展症状（ext）信号，
  /// 供 v3.2 公式的 extRequired/extReference 命中。仅接入有可靠布尔对应的键，避免误映射。
  void _syncExtSymptomsFromAnswers() {
    if (_answers['edema'] == true) _extSymptoms.add('edema');
    if (_answers['cough'] == true || _answers['asthma'] == true) {
      _extSymptoms.add('cough_wheeze');
    }
    if (_answers['neck_stiff'] == true) _extSymptoms.add('neck_stiff');
    if (_answers['jaundice'] == true) _extSymptoms.add('jaundice');
  }
  ...
  DiagnosisResult? diagnose() {
    ...
    // 架构修正：最终诊断前，用【全部】答案（十问 + 跟进）重算「合病」，
    // 使跟进问诊携带的少阳决定性信号（口苦咽干）能修正合病方剂
    // （柴胡桂枝汤）。注意：不重算「定经」（_decideMeridianDirection），
    // 以免把已正确定向的太阳证（大青龙/小青龙）误判；少阴↔厥阴纠偏见下方。
    _detectCombinedPattern();
    _syncExtSymptomsFromAnswers(); // 喂扩展症状给 v3.2 规则引擎

    // ===== 主引擎：v3.2 公式文件（用户指定以所发文件为主） =====
    ...
    if (ruleResult.formula.isNotEmpty) {
      ...
      return DiagnosisResult(
        ...
        recommendConsult: ruleResult.recommendConsult,
        answers: Map.from(_answers),
    ...
    // ===== 兜底：原硬编码六经/杂病逻辑（规则引擎信号不足时） =====
    var meridian = _meridianDirection!;
  ...
  DiagnosisResult _diagnoseTaiYin(Map<String, dynamic> answers) {
    ...
    if (_symSelected('能吃但腹胀') || answers['abdominal_distension'] == true) {
      return DiagnosisResult(
        meridian: '太阴',
  ...
  DiagnosisResult? _diagnoseMiscellaneous(Map<String, dynamic> answers) {
    ...
    // 红蓝花酒：妇人腹中血气刺痛（金匮·妇人杂病）
    if (_symSelected('腹中血气刺痛') || _symSelected('血气刺痛')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '妇人血气刺痛（红蓝花酒证）',
        patternDetail: '妇人六十二种风，及腹中血气刺痛。',
        formula: '红蓝花酒',
        explanation: '红蓝花（红花）一味酒煎。活血行瘀，治妇人腹中血气刺痛。',
        confidence: 0.82,
        matchedSymptoms: _selectedSymptoms,
      );
    }
    ...
    if (_symSelected('项背强') && answers['has_sweat'] != true) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '太阳阳明合病（葛根汤证）',
    ...
    // 葛根加半夏汤：太阳阳明合病呕
    if (_symSelected('项背强') && answers['vomiting'] == true) {
      return DiagnosisResult(
        meridian: '太阳',
    ...
    // 麻黄杏仁薏苡甘草汤：风湿一身尽疼
    if (_symSelected('一身尽疼') && _answers['fever'] == true) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '风湿身疼（麻杏薏甘汤证）',
    ...
    // 桂枝二麻黄一汤：形似疟日再发
    if (_symSelected('日再发') || _symSelected('似疟')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '中风轻证（桂二麻一汤证）',
    ...
    // 桂枝二越婢一汤：发热恶寒热多寒少
    if (_symSelected('热多寒少') && _pulseType == '微弱') {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '太阳轻证（桂二越一汤证）',
    ...
    // 奔豚汤（奔豚+往来寒热）。加专属词排除。
    // FIX-P2-3: 再加 气上冲胸 排除——苓桂术甘汤（心下逆满气上冲胸）专属触发词，
    // '气上冲胸' 含子串 '气上冲'，会误命中桂枝加桂。
    if ((_symSelected('奔豚') || _symSelected('气上冲')) &&
        !_symSelected('胸中痞硬') &&
        !_symSelected('往来寒热') &&
        !_symSelected('气上冲胸')) {
      return DiagnosisResult(
    ...
    if (_symSelected('心下满微痛') && answers['urine_difficult'] == true) {
      return DiagnosisResult(
        meridian: '太阳',
    ...
    if (_symSelected('惊狂') ||
        (_symSelected('卧起不安') && hasMistreatmentHistoryG)) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '亡阳惊狂（桂枝救逆汤证）',
    ...
    // 小青龙加石膏汤：肺胀烦躁而喘
    if (_symSelected('肺胀') && answers['irritable'] == true) {
      return DiagnosisResult(
        meridian: '太阳',
    ...
    // 三物小白散：寒实结胸
    if (_symSelected('结胸') && answers['cold_limbs'] == true) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '寒实结胸（三物小白散证）',
    ...
    // 苓桂甘枣汤：脐下悸欲作奔豚
    if (_symSelected('脐下悸') || _symSelected('欲作奔豚')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '奔豚欲作（苓桂甘枣汤证）',
    ...
    // 茯苓甘草汤：厥而心下悸
    if ((answers['cold_limbs'] == true || _symSelected('厥而心下悸')) &&
        _symSelected('心下悸')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '水饮厥逆（茯苓甘草汤证）',
    ...
    // 瓜蒂散：胸中痞硬气上冲喉咽
    if (_symSelected('胸中痞硬') && _symSelected('气上冲')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '痰实胸中（瓜蒂散证）',
    ...
    // 十枣汤：悬饮（心下痞硬满引胁下痛）
    if (_symSelected('心下痞硬') && _symSelected('引胁下痛')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '悬饮（十枣汤证）',
    ...
    if ((_symSelected('心中懊憹') || _symSelected('反复颠倒')) &&
      ...
      if (_symSelected('少气') || _symSelected('虚羸少气')) {
        return DiagnosisResult(
          meridian: '阳明',
          pattern: '虚烦兼少气（栀子甘草豉汤证）',
      ...
      // 裸心中懊憹：留基础方栀子豉汤（下方 4855 兜底）
    }

    // 猪胆汁方/蜜煎导：津亏内竭，大便硬不可攻（伤寒导法）
    if (_symSelected('津亏') || _symSelected('自欲大便')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '津亏便秘（猪胆汁方/蜜煎导证）',
        patternDetail: '津液内竭，虽硬不可攻之，当须自欲大便，宜蜜煎导、猪胆汁皆可为导。',
        formula: '猪胆汁方',
        explanation: '蜜煎导/大猪胆汁灌肠润燥。津伤肠燥、不可妄攻之导法。',
        confidence: 0.8,
        matchedSymptoms: _selectedSymptoms,
      );
    }
    ...
    if (answers['constipated'] == true && _symSelected('小便数')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '脾约（麻子仁丸证）',
    ...
    // 竹叶石膏汤：伤寒解后虚羸少气
    if (_symSelected('虚羸少气') || _symSelected('气逆欲吐')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '余热未清（竹叶石膏汤证）',
    ...
    // 厚朴七物汤：腹满发热脉浮数
    if (_symSelected('腹满') && _answers['fever'] == true && _pulseType == '浮') {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '表里双解（厚朴七物汤证）',
    ...
    // 大陷胸丸：结胸项亦强如柔痉状
    if (_symSelected('结胸') && _symSelected('项强')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '结胸项强（大陷胸丸证）',
    ...
    // 千金三黄汤：中风手足拘急
    if (_symSelected('手足拘急') && _symSelected('百节疼痛')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '中风历节（千金三黄汤证）',
    ...
    // 小续命汤：中风卒然不知人
    if (_symSelected('中风') && _symSelected('半身不遂')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '中风（小续命汤证）',
    ...
    // 矾石汤：脚气冲心（金匮·中风历节）
    if (_symSelected('脚气冲心')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '脚气冲心（矾石汤证）',
        patternDetail: '脚气冲心，矾石汤主之。',
        formula: '矾石汤',
        explanation: '矾石一味水煎渍脚。收涩下注之湿，治脚气冲心。',
        confidence: 0.8,
        matchedSymptoms: _selectedSymptoms,
      );
    }
    ...
    // 桂枝芍药知母汤：诸肢节疼痛尪羸
    if (_symSelected('肢节疼痛') && _symSelected('脚肿')) {
      return DiagnosisResult(
        meridian: '太阳',
    ...
    // 桂枝附子汤：风湿相搏身体疼烦
    if (_symSelected('身体疼烦') && _symSelected('不能自转侧')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '风湿相搏（桂枝附子汤证）',
    ...
    // 甘草附子汤：风湿骨节疼烦掣痛
    if (_symSelected('骨节疼烦') && _symSelected('掣痛')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '风湿骨节（甘草附子汤证）',
    ...
    // 防己茯苓汤：皮水四肢肿聂聂动
    if (_symSelected('四肢肿') && _symSelected('聂聂动')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '皮水（防己茯苓汤证）',
    ...
    // 越婢汤：风水恶风一身悉肿
    if (_symSelected('风水') && _symSelected('一身悉肿')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '风水（越婢汤证）',
    ...
    // 越婢加术汤：里水身肿小便不利
    if (_symSelected('身肿') && answers['urine_difficult'] == true) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '里水（越婢加术汤证）',
    ...
    // 甘草麻黄汤：皮水身面浮肿
    if (_symSelected('身面浮肿') && answers['has_sweat'] != true) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '皮水（甘草麻黄汤证）',
    ...
    // 木防己汤：膈间支饮心下痞坚
    if (_symSelected('膈间支饮') || _symSelected('心下痞坚')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '膈间支饮（木防己汤证）',
    ...
    // 薏苡附子散：胸痹缓急
    if (_symSelected('胸痹') && _symSelected('缓急')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '胸痹缓急（薏苡附子散证）',
    ...
    // 桂枝生姜枳实汤：心中痞诸逆心悬痛
    if (_symSelected('心中痞') && _symSelected('心悬痛')) {
      return DiagnosisResult(
        meridian: '太阴',
    ...
    // 乌头赤石脂丸：心痛彻背背痛彻心
    if (_symSelected('心痛彻背') || _symSelected('背痛彻心')) {
      return DiagnosisResult(
        meridian: '少阴',
    ...
    // 大建中汤：心胸中大寒痛呕不能食
    if (_symSelected('心胸中大寒痛') || _symSelected('上冲皮起')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '虚寒腹痛（大建中汤证）',
    ...
    // 附子粳米汤：腹中寒气雷鸣切痛
    if (_symSelected('雷鸣切痛') || _symSelected('腹中寒气')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '腹中寒气（附子粳米汤证）',
    ...
    if (_symSelected('绕脐痛') &&
      ...
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '寒疝绕脐（大乌头煎证）',
        patternDetail: '寒疝绕脐痛，若发则白汗出，手足厥冷，其脉沉紧。',
        formula: '大乌头煎',
        explanation: '乌头大者熬去皮，水煎去滓纳蜜。散寒止痛，治寒疝绕脐剧痛。',
        confidence: 0.85,
        matchedSymptoms: _selectedSymptoms,
      );
    }
    ...
    // 当归生姜羊肉汤：寒疝腹中痛胁痛里急
    if (_symSelected('寒疝') && _symSelected('胁痛里急')) {
      return DiagnosisResult(
        meridian: '厥阴',
    ...
    // 乌头桂枝汤：寒疝腹中痛逆冷手足不仁
    if (_symSelected('寒疝') && _symSelected('手足不仁')) {
      return DiagnosisResult(
        meridian: '厥阴',
    ...
    // 蜘蛛散：阴狐疝气
    if (_symSelected('狐疝') || _symSelected('偏有大小')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '狐疝（蜘蛛散证）',
    ...
    // 越婢加半夏汤：肺胀喘目如脱状
    if (_symSelected('肺胀') && _symSelected('目如脱状')) {
      return DiagnosisResult(
        meridian: '太阳',
    ...
    // 葶苈大枣泻肺汤：肺痈喘不得卧
    if (_symSelected('肺痈') && _symSelected('喘不得卧')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '肺痈（葶苈大枣泻肺汤证）',
    ...
    // 茯苓杏仁甘草汤：胸痹胸中气塞短气
    if (_symSelected('胸痹') && _symSelected('短气')) {
      return DiagnosisResult(
        meridian: '太阴',
    ...
    // 枳术汤：心下坚大如盘
    if (_symSelected('心下坚') && _symSelected('大如盘')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '水饮痞坚（枳术汤证）',
    ...
    // 四逆加人参汤：霍乱吐利止后恶寒脉微
    if (_symSelected('霍乱') || _symSelected('吐利止')) {
      return DiagnosisResult(
        meridian: '少阴',
    ...
    // 通脉四逆汤：少阴下利清谷里寒外热
    if (answers['diarrhea'] == true && _symSelected('里寒外热')) {
      return DiagnosisResult(
        meridian: '少阴',
        pattern: '阴盛格阳（通脉四逆汤证）',
    ...
    // 通脉四逆加猪胆汁汤：吐已下断脉微欲绝
    if (_symSelected('吐已下断') || _symSelected('四肢拘急')) {
      return DiagnosisResult(
        meridian: '少阴',
    ...
    // 牡蛎泽泻散：大病差后腰以下水气
    if (_symSelected('大病差后') && _symSelected('腰以下水气')) {
      return DiagnosisResult(
        meridian: '少阴',
        pattern: '病后水气（牡蛎泽泻散证）',
    ...
    // 茵陈五苓散：黄疸（湿重于热）
    if (_symSelected('黄疸') && answers['urine_difficult'] == true) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '黄疸湿重（茵陈五苓散证）',
    ...
    // 硝石矾石散：女劳疸（膀胱急少腹满）
    if (_symSelected('女劳') || _symSelected('膀胱急')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '女劳疸（硝石矾石散证）',
    ...
    // 猪膏髪煎：诸黄
    if (_symSelected('诸黄') || _symSelected('黄疸通用')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '黄疸通治（猪膏髪煎证）',
    ...
    // 柏叶汤：吐血不止
    if (_symSelected('吐血') || _symSelected('吐血不止')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '吐血（柏叶汤证）',
        patternDetail: '吐血不止者。',
    ...
    // 泻心汤：心气不足吐血衄血
    if (_symSelected('吐血衄血') || _symSelected('心气不足')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '热盛吐衄（泻心汤证）',
    ...
    // 黄土汤：远血（先便后血）
    if (_symSelected('先便后血') || _symSelected('远血')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '脾虚便血（黄土汤证）',
    ...
    // 赤小豆当归散：目赤如鸠眼（狐惑病）。FIX: 排除「蚀于肛」，避免遮蔽雄黄熏方
    if (_symSelected('目赤如鸠眼') && !_symSelected('蚀于肛')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '狐惑病（赤豆当归散证）',
    ...
    // 防己地黄汤：病如狂状妄行独语不休
    if (_symSelected('如狂') && _symSelected('独语不休')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '肝火如狂（防己地黄汤证）',
    ...
    // 大黄蟅虫丸：五劳虚极羸瘦内有干血
    if (_symSelected('肌肤甲错') || _symSelected('两目黯黑')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '干血劳（大黄蟅虫丸证）',
    ...
    if ((_symSelected('产后腹痛') || _symSelected('干血着脐下')) &&
        !_symSelected('烦满不得卧')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '产后瘀血（下瘀血汤证）',
    ...
    // 大黄甘遂汤：水与血俱结在血室
    if (_symSelected('水与血俱结') || _symSelected('血室')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '水血互结（大黄甘遂汤证）',
    ...
    // 血虚兼血瘀：桂枝茯苓丸（仲景活血化瘀主方，代后世桃红四物汤）
    if (_symSelected('血虚兼血瘀') || _symSelected('面色萎黄')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '血虚血瘀（桂枝茯苓丸证）',
        patternDetail: '血虚兼血瘀，面色萎黄，胸胁刺痛。瘀血内阻。',
        formula: '桂枝茯苓丸',
        explanation: '桂枝芍药桃仁丹皮茯苓。活血化瘀、养血利水，仲景活血化瘀主方。',
        confidence: 0.8,
        matchedSymptoms: _selectedSymptoms,
      );
    }
    ...
    // 桂枝茯苓丸：妇人宿有症病漏下
    if (_symSelected('症病') || _symSelected('漏下不止')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '症瘕（桂枝茯苓丸证）',
    ...
    // 当归散：妊娠养胎
    if (_symSelected('妊娠') && _symSelected('养胎')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '妊娠养胎（当归散证）',
    ...
    // 白术散：妊娠养胎
    if (_symSelected('妊娠') && _symSelected('白术散')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '妊娠寒湿（白术散证）',
    ...
    // 胶姜汤：妇人陷经漏下黑不解
    if (_symSelected('漏下黑不解') || _symSelected('陷经')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '虚寒漏下（胶姜汤证）',
    ...
    // 矾石丸：妇人经水闭不利中有干血
    if (_symSelected('经水闭不利') || _symSelected('下白物')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '干血内停（矾石丸证）',
    ...
    // 土瓜根散：带下经水不利少腹满痛
    if (_symSelected('经水不利') && _symSelected('少腹满痛')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '瘀血经不利（土瓜根散证）',
    ...
    // 竹叶汤：产后中风
    if (_symSelected('产后中风') || _symSelected('产后发热')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '产后中风（竹叶汤证）',
    ...
    // 奔豚汤：奔豚气上冲胸腹痛往来寒热
    if (_symSelected('奔豚') && _symSelected('往来寒热')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '奔豚（奔豚汤证）',
    ...
    // 百合洗方：百合病一月不解变成渴（金匮·百合狐惑）
    if ((_symSelected('百合病') || _symSelected('意欲食复不能食')) &&
        (_symSelected('渴不差') || _symSelected('变成渴') || _symSelected('一月不解'))) {
      return DiagnosisResult(
        meridian: '少阴',
        pattern: '百合病渴（百合洗方证）',
        patternDetail: '百合病一月不解变成渴者，百合洗方主之。',
        formula: '百合洗方',
        explanation: '百合渍水一宿，洗身。外洗清热，治百合病渴不差。',
        confidence: 0.8,
        matchedSymptoms: _selectedSymptoms,
      );
    }
    ...
    if (_symSelected('蚀于肛')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '狐惑蚀肛（雄黄熏方证）',
        patternDetail: '蚀于肛者，雄黄熏之。',
        formula: '雄黄熏方',
        explanation: '雄黄烧烟熏肛。燥湿杀虫，治狐惑蚀于肛。',
        confidence: 0.78,
        matchedSymptoms: _selectedSymptoms,
      );
    }
    ...
    // 百合知母汤：百合病发汗后
    if (_symSelected('百合病') && _symSelected('发汗后')) {
      return DiagnosisResult(
        meridian: '太阴',
    ...
    // 百合滑石代赭汤：百合病下之后
    if (_symSelected('百合病') && _symSelected('下之后')) {
      return DiagnosisResult(
        meridian: '太阴',
    ...
    // 百合滑石散：百合病变发热
    if (_symSelected('百合病') && _symSelected('发热')) {
      return DiagnosisResult(
        meridian: '太阴',
    ...
    // 百合鸡子黄汤：百合病吐之后
    if (_symSelected('百合病') && _symSelected('吐之后')) {
      return DiagnosisResult(
        meridian: '太阴',
    ...
    if (_symSelected('浸淫疮')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '浸淫疮（黄连粉证）',
        patternDetail: '浸淫疮，黄连粉主之。',
        formula: '黄连粉',
        explanation: '黄连一味为粉敷之。清热燥湿，治浸淫疮遍身。',
        confidence: 0.8,
        matchedSymptoms: _selectedSymptoms,
      );
    }
    ...
    if ((_symSelected('肠痈') || _symSelected('腹皮急按之濡')) &&
        !_symSelected('脓已成')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '肠痈（大黄牡丹汤证）',
    ...
    // 薏苡附子败酱散：肠痈脓已成
    if (_symSelected('肠痈') && _symSelected('脓已成')) {
      return DiagnosisResult(
        meridian: '阳明',
    ...
    // 排脓散/排脓汤：疮痈脓已成
    if (_symSelected('疮痈') && _symSelected('脓已成')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '疮痈排脓（排脓散/汤证）',
        patternDetail: '疮痈脓已成。',
    ...
    // 升麻鳖甲汤：阳毒面赤斑斑如锦纹
    if (_symSelected('阳毒') || _symSelected('面赤斑斑如锦纹')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '阳毒（升麻鳖甲汤证）',
    ...
    // 小半夏汤：诸呕吐谷不得下
    if (answers['vomiting'] == true && _symSelected('谷不得下')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '痰饮呕吐（小半夏汤证）',
    ...
    if (answers['vomiting'] == true && _symSelected('膈间有水')) {
      return DiagnosisResult(
        meridian: '太阴',
    ...
    // 半夏干姜散：干呕吐逆吐涎沫
    if (answers['vomiting'] == true && _symSelected('吐涎沫')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '胃寒呕吐（半夏干姜散证）',
    ...
    // 生姜半夏汤：似喘不喘似呕不呕
    if (_symSelected('似喘不喘') || _symSelected('彻心中愦愦然无奈')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '痰饮郁膈（生姜半夏汤证）',
    ...
    // 橘皮竹茹汤：哕逆
    if (_symSelected('哕逆') || _symSelected('呃逆')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '胃虚哕逆（橘皮竹茹汤证）',
        patternDetail: '哕逆者。',
    ...
    // 茯苓泽泻汤：胃反吐下渴欲饮水（渴+胃反）
    if ((_symSelected('胃反') || answers['vomit_cyclical'] == true) &&
        (answers['thirsty'] == true || _symSelected('渴'))) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '水饮胃反（茯苓泽泻汤证）',
    ...
    // 大黄甘草汤：食已即吐
    if (_symSelected('食已即吐') || answers['vomit_immediate'] == true) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '胃肠实热（大黄甘草汤证）',
        patternDetail: '食已即吐者。',
    ...
    // 赤石脂禹余粮汤：下利不止心下痞硬
    if (answers['diarrhea'] == true && _symSelected('下利不止')) {
      return DiagnosisResult(
        meridian: '太阴',
    ...
    // 诃黎勒散：气利
    if (_symSelected('气利') || _symSelected('矢气时大便随之而出')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '气利（诃黎勒散证）',
    ...
    // 紫参汤：下利肺痛
    if (answers['diarrhea'] == true && _symSelected('肺痛')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '下利肺痛（紫参汤证）',
        patternDetail: '下利肺痛者。',
    ...
    // 白头翁加甘草阿胶汤：产后下利虚极
    if (_symSelected('产后下利') || _symSelected('下利虚极')) {
      return DiagnosisResult(
        meridian: '厥阴',
    ...
    // 麦门冬汤：火逆上气咽喉不利
    if (_symSelected('火逆上气') || _symSelected('咽喉不利')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '虚火上炎（麦门冬汤证）',
    ...
    // 甘草粉蜜汤：蛔虫吐涎心痛
    if (_symSelected('蛔虫') || _symSelected('心痛发作有时')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '蛔虫（甘草粉蜜汤证）',
    ...
    // 鸡矢白散：转筋入腹
    if (_symSelected('转筋') || _symSelected('臂脚直')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '转筋（鸡矢白散证）',
    ...
    // 旋覆花汤：肝着常欲蹈其胸上
    if (_symSelected('肝着') || _symSelected('常欲蹈其胸上')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '肝着（旋覆花汤证）',
    ...
    // 枳实芍药散：产后腹痛烦满不得卧
    if (_symSelected('产后腹痛') && _symSelected('烦满不得卧')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '产后气滞（枳实芍药散证）',
    ...
    // 鳖甲煎丸：疟母
    if (_symSelected('疟母') || _symSelected('脾脏肿大')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '疟母（鳖甲煎丸证）',
    ...
    // 附子泻心汤：心下痞恶寒汗出
    if (_symSelected('心下痞') && answers['cold_limbs'] == true) {
      return DiagnosisResult(
        meridian: '阳明',
    ...
    // 柴胡桂枝汤：发热微恶寒支节烦疼微呕
    if (_symSelected('支节烦疼') && answers['vomiting'] == true) {
      return DiagnosisResult(
        meridian: '少阳',
    ...
    // 柴胡桂枝干姜汤：胸胁满微结小便不利
    if (_symSelected('胸胁满微结') && answers['urine_difficult'] == true) {
      return DiagnosisResult(
        meridian: '少阳',
    ...
    // 柴胡加芒硝汤：日晡所发潮热
    if (_symSelected('潮热') && _symSelected('胸胁满')) {
      return DiagnosisResult(
        meridian: '少阳',
    ...
    // 栀子枳实汤：大下后身热不去心中结痛
    if (_symSelected('心中结痛') && _answers['fever'] == true) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '热郁胸膈（栀子枳实汤证）',
    ...
    // 文蛤散：渴欲饮水不止
    if (answers['thirsty'] == true && _symSelected('饮水不止')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '消渴（文蛤散证）',
    ...
    // 枳实栀子豉汤：大病差后劳复
    if (_symSelected('劳复') || _symSelected('大病差后')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '劳复（枳实栀子豉汤证）',
    ...
    // 厚朴大黄汤：支饮胸满
    if (_symSelected('支饮') && _symSelected('胸满')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '支饮胸满（厚朴大黄汤证）',
        patternDetail: '支饮胸满者。',
    ...
    // 苓甘五味姜辛汤系列：痰饮咳嗽
    if (_symSelected('痰饮咳嗽') || _symSelected('咳满')) {
      return DiagnosisResult(
        meridian: '太阴',
    ...
    // 滑石白鱼散：小便不利
    if (answers['urine_difficult'] == true && _symSelected('白鱼')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '小便不利（滑石白鱼散证）',
    ...
    // 茯苓戎盐汤：小便不利
    if (answers['urine_difficult'] == true && _symSelected('戎盐')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '小便不利（茯苓戎盐汤证）',
    ...
    // 葵子茯苓散：妊娠水气身重小便不利
    if (_symSelected('妊娠') && answers['urine_difficult'] == true) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '妊娠水肿（葵子茯苓散证）',
    ...
    // 当归贝母苦参丸：妊娠小便难
    if (_symSelected('妊娠') && _symSelected('小便难')) {
      return DiagnosisResult(
        meridian: '太阴',
    ...
    // 己椒苈黄丸：腹满口舌干燥肠间有水气
    if (_symSelected('腹满') && _symSelected('口舌干燥')) {
      return DiagnosisResult(
        meridian: '阳明',
        pattern: '肠间水气（己椒苈黄丸证）',
    ...
    // 甘遂半夏汤：留饮欲去利反快
    if (_symSelected('留饮') || _symSelected('利反快')) {
      return DiagnosisResult(
        meridian: '太阴',
        pattern: '留饮（甘遂半夏汤证）',
    ...
    // 侯氏黑散：大风四肢烦重
    if (_symSelected('四肢烦重') || _symSelected('大风')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '大风（侯氏黑散证）',
    ...
    // 风引汤：大人风引少小惊痫
    if (_symSelected('惊痫') || _symSelected('瘛疭')) {
      return DiagnosisResult(
        meridian: '厥阴',
        pattern: '风痫（风引汤证）',
    ...
    // 赤丸：寒气厥逆
    if (_symSelected('寒气厥逆') || _symSelected('赤丸')) {
      return DiagnosisResult(
        meridian: '少阴',
        pattern: '寒气厥逆（赤丸证）',
        patternDetail: '寒气厥逆。',
    ...
    // 术附汤：排脓（末期乳癌）
    if (_symSelected('排脓') && _symSelected('乳癌')) {
      return DiagnosisResult(
        meridian: '少阴',
        pattern: '排脓（术附汤证）',
    ...
    // 附子散：小儿足烂疮阴证疮疡
    if (_symSelected('足烂疮') || _symSelected('阴证疮疡')) {
      return DiagnosisResult(
        meridian: '少阴',
        pattern: '阴证疮疡（附子散证）',
    ...
    // 栀子豉汤：虚烦不得眠心中懊憹（条文84）—基础方，裸心中懊憹/反复颠倒即命中
    if (_symSelected('心中懊憹') || _symSelected('反复颠倒')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '余热虚烦（栀子豉汤证）',
    ...
    if (_symSelected('身痒') && _symSelected('热多寒少') ||
        (answers['itchy_skin'] == true && answers['no_sweat'] == true)) {
      return DiagnosisResult(
        meridian: '太阳',
    ...
    // 杏子汤：身肿喘（水气篇，原方方阙失传，倪师拟=麻黄杏仁甘草汤，无石膏）
    if (_symSelected('身肿') && _symSelected('喘')) {
      return DiagnosisResult(
        meridian: '太阳',
        pattern: '风水身肿喘（杏子汤证·方阙）',
        patternDetail: '水气病，身肿而喘，杏子汤主之（原方失传，倪师拟=麻黄杏仁甘草汤，无石膏）。',
        formula: '杏子汤',
        explanation: '麻黄杏仁甘草。表水身肿兼喘；杏子汤方阙，倪师以麻黄杏仁甘草汤代之（非麻杏甘石汤）。',
        confidence: 0.78,
        matchedSymptoms: _selectedSymptoms,
      );
    }
    ...
    // 牡蛎汤：牝疟（外台补充方，倪师《金匮》附方，治牝疟）。由 UI '（→牡蛎汤）' 选项触发
    if (_symSelected('牡蛎汤')) {
      return DiagnosisResult(
        meridian: '少阳',
        pattern: '牝疟（牡蛎汤证）',
        patternDetail: '治牝疟（寒多者），牡蛎汤主之（外台秘要补充方）。',
        formula: '牡蛎汤',
        explanation: '牡蛎麻黄甘草蜀漆。疟多寒，寒痰阻遏，牡蛎咸以软坚、降浮阳。',
        confidence: 0.75,
        matchedSymptoms: _selectedSymptoms,
      );
    }
