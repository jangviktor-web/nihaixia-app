...
> ### ⚠️ 升级须知：安装前请先卸载旧版本
>
> **自 V1.11.9 起，APK 改由 GitHub Actions 云端构建；V1.11.8 及更早的版本均在开发者本地机器构建。两者使用的是不同的签名密钥，Android 会将其判定为两个不同的应用——若直接覆盖安装，系统会提示「应用未安装」或「安装失败」。**
>
> | 版本范围 | 构建环境 | 签名来源 |
> |:---|:---|:---|
> | V1.11.8 及更早 | 开发者本地机器 | 本地正式签名（`release-key.jks`，别名 `nihaisha`） |
> | V1.11.9 及以后 | GitHub Actions 云端 | 云端自动生成的调试签名（仓库暂未配置签名密钥） |
>
> **正确的升级顺序：**
> 1. **先备份**（建议）：App「设置」→ 导出收藏与诊断历史，避免卸载后数据丢失；
> 2. **卸载旧版本**：系统设置 → 应用管理 → 汉唐中医 → 卸载；
> 3. **安装新版本**：下载下方 APK 并安装（首次需允许「未知来源」）。
>
> 卸载仅清除本 App 的数据，不会影响手机上的其他内容。后续版本计划把签名密钥配置为 GitHub Secrets，届时可恢复**直接覆盖安装、无需卸载**的无缝升级。
...
多半是**签名不一致**导致的覆盖安装被拒。自 V1.11.9 起 APK 由 GitHub Actions 云端构建，而 V1.11.8 及更早的版本在开发者本地机器构建，两者签名密钥不同，Android 视为两个不同的应用，无法直接覆盖。解决办法：**先卸载旧版本，再安装新版本**（卸载前记得在「设置」中导出收藏与诊断历史）。详见上方「⚠️ 升级须知」。
