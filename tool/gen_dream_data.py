...
def main() -> None:
    ...
    out.append('  Iterable<DreamEntry> list = kDreamEntries;')
    out.append('  if (category != null) {')
    out.append("    list = list.where((e) => e.category == category);")
    out.append('  }')
    out.append('  if (q.isEmpty) return list.toList();')
    out.append('  return list')
    out.append('      .where((e) =>')
    out.append('          e.keyword.toLowerCase().contains(q) ||')
    out.append('          e.dream.toLowerCase().contains(q) ||')
    out.append('          e.interpretation.toLowerCase().contains(q))')
    out.append('      .toList();')
    out.append('}')
    out.append('')
